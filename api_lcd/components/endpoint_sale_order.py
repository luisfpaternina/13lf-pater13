...
class SaleOrder(Component):
    ...
    def create(self, **params):
        ...
        invoice.action_post()
        vals_delivery = {}
        vals_delivery['partner_id'] = params["partner_id"]
        for item in params["order_lines"]:
            if item['entrega_tienda'] > 0:
                for sales in sale.order_line:
                    vals_delivery['line_ids']=[(0,0,{
                        "order_line_id": sales.id,
                        "name": sales.product_id.name,
                        "product_id": item['product_id'],
                        "qty_ordered": item['product_uom_qty'],
                        "qty_procured": 0,
                        "quantity": item['entrega_tienda']
                    })]
                    obj_delivery=self.env['manual.delivery'].with_context(active_ids=[sales.id],active_model='sale.order.line').create(vals_delivery)
                ...
                "order_lines": {"type":"list",
                                "schema": {"type": "dict",
                                        "schema": {
                                            "product_id": {"type":"integer", "required": False},
                                            "product_uom_qty": {"type":"float", "required": False},
                                             "entrega_tienda": {"type":"integer", "required": True}
                                                  }
                                          }
                               }
              }
